...
class AddressPickerViewController: ListingManager {
    ...
    @IBAction func onFilterChanged(_ sender: UITextField) {
        guard let filter = sender.text else {
            return
        }
        
        let debounceHandler: () -> Void = { [weak self] in
            if filter.isEmpty || filter.count < 3 {
                return
            }
...
extension AddressPickerViewController {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.setData(data: addresses[indexPath.row])
        return cell
    }
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        addressPickCompletion?(addresses[indexPath.row])
